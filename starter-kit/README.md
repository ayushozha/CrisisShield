...
## Version 1 - Hosted LLM path

You can start with this before the hackathon, if you want to. Or test the hosted LLM path and Nemotron side-by-side during the hackathon, using Cekura.

This bot only requires a Gradium API key and an LLM provider key. Sign up for free at [Gradium](https://gradium.ai). We'll provide a code for Gradium credits, during the event.

- **STT:** [Gradium](https://gradium.ai)
- **LLM:** Hosted Responses-compatible model configured by environment
...
- API keys for the hosted LLM provider and [Gradium](https://gradium.ai)
...
2. **Configure API keys:**
   ...
   ```bash
   cp .env.example .env
   # Edit .env and fill in the LLM provider key and GRADIUM_API_KEY.
