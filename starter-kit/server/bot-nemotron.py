...
async def run_bot(
    ...
    initial_offer_text = (
        "Sorry, no one is available to answer your call right now. "
        "Would you like to speak with our specialized agent?"
    )
    ...
    if customer:
        caller_context = (
            ...
            "Do not use their name or mention their last order in the greeting; "
            "that comes across as surveilling. Only after they opt in and say they want flowers, you "
        ...
        caller_context = (
            "You're talking to a new customer. Do not introduce the shop until they opt in."
        )
    ...
    system_instruction = (
        ...
        "Inbound call gate:\n"
        f'- Your first spoken line on every call must be exactly: "{initial_offer_text}"\n'
        "- Until the caller clearly agrees to speak with the specialized agent, do not "
        "discuss flowers, orders, the shop, prices, delivery, or caller history.\n"
        '- If the caller says yes, yeah, sure, okay, please, "I need help", or otherwise '
        'accepts, say: "I can help. What can I do for you today?" Then continue as the '
        "Field & Flower voice agent.\n"
        '- If the caller says no, no thanks, not interested, "I do not want to talk with '
        'anyone", stop calling me, wrong number, goodbye, or otherwise declines, say a '
        'short goodbye such as "No problem. Goodbye." and call end_call in that same turn.\n'
        '- If their answer is unclear, ask once: "Would you like to speak with the '
        'specialized agent?" If they still do not clearly accept, say goodbye and call '
        "end_call.\n\n"
    ...
    async def on_client_connected(transport, client):
        ...
        await worker.queue_frames([TTSSpeakFrame(initial_offer_text, append_to_context=True)])
